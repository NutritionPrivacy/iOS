...
final class GoalPaceQuestionViewController: UIViewController, OnboardingFooterHosting {
    private let viewModel: OnboardingViewModel
    private let step: OnboardingStep = .goalPace
    private let rootView = UIView()
    private let headerView = OnboardingHeaderView()
    private let contentContainerView = UIView()
    private let messageLabel = UILabel()
    let footerView = OnboardingFooterView()
    private let optionsView = OptionButtonListView(options: GoalPace.allCases) { String(localized: $0.title) }
    ...
    private func configureQuestionContent() {
        headerView.titleLabel.text = "How fast should progress move?"
        headerView.subtitleLabel.text = "Choose a pace that feels realistic for your routine."
        footerView.primaryButton.configuration?.title = "Continue"
        optionsView.onSelection = { [weak self] selection in
            self?.viewModel.draft.goalPace = selection
        }
    }
    ...
    private func applyModelToView() {
        ...
        optionsView.setSelectedOption(viewModel.draft.goalPace)
    }
...
#if DEBUG
#Preview {
    UINavigationController(rootViewController: GoalPaceQuestionViewController(
        viewModel: OnboardingPreviewSupport.makeViewModel(currentStep: .goalPace)
    ))
}
#endif
