...
final class NameQuestionViewController: UIViewController, OnboardingFooterHosting {
    ...
    private func configureQuestionContent() {
        headerView.titleLabel.text = "What should we call you?"
        headerView.subtitleLabel.text = "This name is only stored on your device."
        footerView.primaryButton.configuration?.title = "Continue"

        inputViewControl.setPlaceholder("Your name")
...
#if DEBUG
#Preview {
    UINavigationController(rootViewController: NameQuestionViewController(
        viewModel: OnboardingPreviewSupport.makeViewModel(currentStep: .name)
    ))
}
#endif
