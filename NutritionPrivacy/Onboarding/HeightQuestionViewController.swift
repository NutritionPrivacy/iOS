...
final class HeightQuestionViewController: UIViewController, OnboardingFooterHosting {
    private let viewModel: OnboardingViewModel
    private let step: OnboardingStep = .height
    private let rootView = UIView()
    private let headerView = OnboardingHeaderView()
    private let inputContainerView = UIView()
    private let messageLabel = UILabel()
    let footerView = OnboardingFooterView()
    private let inputViewControl = NumericQuestionInputView()
    ...
    private func configureQuestionContent() {
        ...
        inputViewControl.configure(
            placeholder: "Height",
            unitTitles: HeightUnit.allCases.map { String(localized: $0.title) },
            onValueChanged: { [weak self] value in
                guard let self else { return }
                self.viewModel.draft.height = BodyHeight(value: value, unit: self.viewModel.draft.height.unit)
            },
            onUnitChanged: { [weak self] index in
                guard let self, let unit = HeightUnit.allCases[safe: index] else { return }
                self.viewModel.draft.height = BodyHeight(value: self.viewModel.draft.height.value, unit: unit)
            }
        )
    }
    ...
    private func applyModelToView() {
        ...
        inputViewControl.render(
            value: viewModel.draft.height.value,
            selectedUnitIndex: HeightUnit.allCases.firstIndex(of: viewModel.draft.height.unit) ?? 0
        )
    }
...
#if DEBUG
#Preview {
    UINavigationController(rootViewController: HeightQuestionViewController(
        viewModel: OnboardingPreviewSupport.makeViewModel(currentStep: .height)
    ))
}
#endif
