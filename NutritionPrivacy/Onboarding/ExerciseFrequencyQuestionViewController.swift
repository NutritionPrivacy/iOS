...
final class ExerciseFrequencyQuestionViewController: UIViewController, OnboardingFooterHosting {
    private let viewModel: OnboardingViewModel
    private let step: OnboardingStep = .exerciseFrequency
    private let rootView = UIView()
    private let headerView = OnboardingHeaderView()
    private let contentContainerView = UIView()
    private let messageLabel = UILabel()
    let footerView = OnboardingFooterView()
    private let optionsView = OptionButtonListView(options: ExerciseFrequency.allCases) { String(localized: $0.title) }
    ...
    private func configureQuestionContent() {
        headerView.titleLabel.text = "How often do you exercise?"
        headerView.subtitleLabel.text = "We use this as a small adjustment on top of daily activity."
        footerView.primaryButton.configuration?.title = "Continue"
        optionsView.onSelection = { [weak self] selection in
            self?.viewModel.draft.exerciseFrequency = selection
        }
    }
    ...
    private func applyModelToView() {
        ...
        optionsView.setSelectedOption(viewModel.draft.exerciseFrequency)
    }
...
#if DEBUG
#Preview {
    UINavigationController(rootViewController: ExerciseFrequencyQuestionViewController(
        viewModel: OnboardingPreviewSupport.makeViewModel(currentStep: .exerciseFrequency)
    ))
}
#endif
