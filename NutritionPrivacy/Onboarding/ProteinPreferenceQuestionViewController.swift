...
final class ProteinPreferenceQuestionViewController: UIViewController, OnboardingFooterHosting {
    private let viewModel: OnboardingViewModel
    private let step: OnboardingStep = .proteinPreference
    private let rootView = UIView()
    private let headerView = OnboardingHeaderView()
    private let contentContainerView = UIView()
    private let messageLabel = UILabel()
    let footerView = OnboardingFooterView()
    private let optionsView = OptionButtonListView(options: ProteinPreference.allCases) { String(localized: $0.title) }
    ...
    private func configureQuestionContent() {
        headerView.titleLabel.text = "How much protein do you want to prioritize?"
        headerView.subtitleLabel.text = "Higher protein shifts your macro split without changing the overall goal."
        footerView.primaryButton.configuration?.title = "Continue"
        optionsView.onSelection = { [weak self] selection in
            self?.viewModel.draft.proteinPreference = selection
        }
    }
    ...
    private func applyModelToView() {
        ...
        optionsView.setSelectedOption(viewModel.draft.proteinPreference)
    }
...
#if DEBUG
#Preview {
    UINavigationController(rootViewController: ProteinPreferenceQuestionViewController(
        viewModel: OnboardingPreviewSupport.makeViewModel(currentStep: .proteinPreference)
    ))
}
#endif
